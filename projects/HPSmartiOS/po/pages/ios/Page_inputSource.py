# coding: utf-8
from projects.HPSmartiOS.po.models.ios.Page_inputSource_model import Page_inputSource_model


'''Touch inputSource go to here'''
class Page_inputSource(Page_inputSource_model):
    def __init__(self, UI):
        self.UI = UI
        if 1 > 1:
            from fwk.object.IosFwk import IosFwk
            self.UI = IosFwk(None)
        Page_inputSource_model.__init__(self)

    def verifyInputScreen(self):
        self.text_automatic().verifyIsShown()
        self.text_documentFeeder().verifyIsShown()
        self.text_scannerGlass().verifyIsShown()
