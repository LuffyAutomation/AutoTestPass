# coding: utf-8
from projects.HPSmartiOS.po.models.ios.Page_color_model import Page_color_model


'''Touch color go to there'''
class Page_color(Page_color_model):
    def __init__(self, UI):
        self.UI = UI
        if 1 > 1:
            from fwk.object.IosFwk import IosFwk
            self.UI = IosFwk(None)
        Page_color_model.__init__(self)

    def verfiyScanSettings(self):
        self.text_color().verifyIsShown()
        self.text_grayscale().verifyIsShown()
