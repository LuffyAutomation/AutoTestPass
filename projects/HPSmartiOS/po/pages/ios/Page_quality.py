# coding: utf-8
from projects.HPSmartiOS.po.models.ios.Page_quality_model import Page_quality_model


'''Touch quality go to there'''
class Page_quality(Page_quality_model):
    def __init__(self, UI):
        self.UI = UI
        if 1 > 1:
            from fwk.object.IosFwk import IosFwk
            self.UI = IosFwk(None)
        Page_quality_model.__init__(self)

    def verfiyScanSettings(self):
        self.text_draft().verifyIsShown()
        self.text_normal().verifyIsShown()
        self.text_best().verifyIsShown()
