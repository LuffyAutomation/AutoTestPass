# coding: utf-8
from projects.HPSmartiOS.po.models.ios.Page_linkPrivacyStatement_model import Page_linkPrivacyStatement_model


'''touch HP Online Privacy Statement'''
class Page_linkPrivacyStatement(Page_linkPrivacyStatement_model):
    def __init__(self, UI):
        self.UI = UI
        if 1 > 1:
            from fwk.object.IosFwk import IosFwk
            self.UI = IosFwk(None)
        Page_linkPrivacyStatement_model.__init__(self)

    # This is function template of how to write your Business Logic.
    def verfiylinkPage(self):
        self.text_privacyStatement().verifyIsShown()
        self.link_privacy().verifyIsShown()
        self.link_siteTerms().verifyIsShown()
        self.test_termsAndConditions().verifyIsShown()
